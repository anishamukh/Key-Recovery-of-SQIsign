...
import ast
import time
# Global debug flag to track when inside SigningKLPT
DEBUG_SIGNING_KLPT = False
...
def RepresentIntegerHeuristic(M, parity=False):
    ...
    def RepresentInteger(M, z, t, parity=False):
        file_path = "RepresentInteger_attack_values.txt"
        #global DEBUG_SIGNING_KLPT
        #print("[DEBUG] DEBUG_SIGNING_KLPT_ACTIVE set to:", DEBUG_SIGNING_KLPT) 
        M_prime = M - p * quadratic_norm(z, t)
        # if DEBUG_SIGNING_KLPT:
        #     print(f"DEBUG [RepresentIntegerHeuristic]: Called with cornacchia value = {M_prime}")
        #     print(f"DEBUG [RepresentIntegerHeuristic]: pf(z,t) value = {quadratic_norm(z, t)}")
        #     print(f"DEBUG [RepresentIntegerHeuristic]: z, t value = {(z, t)}")
        two_squares1 = Cornacchia(M_prime, -ZZ(ω**2))
        # Define the output file to store M_attack values
        #log_file = "M_attack_values.txt" 
        #attack targets
        if DEBUG_SIGNING_KLPT:
            start_time = time.time()
            M_attack = (M - M_prime)//p
            try:
                with open(file_path, "r") as f:
                    data = f.read()
                    M_attack_list = ast.literal_eval(data) if data else []
            ...
            # Write back to file as a list
            #with open(file_path, "w") as f:
                #f.write(str(M_attack_list))

            #print(f"Stored M_attack: {M_attack} in {file_path}")

            #with open(log_file, "a") as file:
                #file.write(f"M_attack: {M_attack}\n")
            #print("M_attack=", M_attack)

            prob_z_t = two_squares(M_attack)
            #print("prob_z_t=", prob_z_t)
            reqd_z_t = None
            # if((prob_z_t[0] == z and prob_z_t[1] == t) or (prob_z_t[0] == t and prob_z_t[1] == z) or (prob_z_t[0] == -z and prob_z_t[1] == -t) or (prob_z_t[0] == -t and prob_z_t[1] == -z) or (prob_z_t[0] == z and prob_z_t[1] == -t) or (prob_z_t[0] == -z and prob_z_t[1] == t) or (prob_z_t[0] == t and prob_z_t[1] == -z) or (prob_z_t[0] == -t and prob_z_t[1] == z)):
            #     reqd_z_t = prob_z_t
            if prob_z_t[0] == z and prob_z_t[1] == t:
                reqd_z_t = (z, t)
            elif prob_z_t[0] == t and prob_z_t[1] == z:
                reqd_z_t = (z, t)
            elif prob_z_t[0] == -z and prob_z_t[1] == -t:
                reqd_z_t = (z, t)
            elif prob_z_t[0] == -t and prob_z_t[1] == -z:
                reqd_z_t = (z, t)
            elif prob_z_t[0] == z and prob_z_t[1] == -t:
                ...
                reqd_z_t = (z, t)
            #print("reqd_z_t=", reqd_z_t)

                #print("attack successful in represent integer")

        if two_squares1:
            x, y = two_squares1
            γ_final = x + ω * y + j * (z + ω * t)
            #print("γ_final=", γ_final)

            # If we identified a valid attack pair, construct γ_constructed
            if DEBUG_SIGNING_KLPT:
                if reqd_z_t:
                    γ_constructed = x + ω * y + j * (reqd_z_t[0] + ω *reqd_z_t[1])
                    #print("γ_constructed=", γ_constructed)

                # Compare constructed γ with final γ
                    if γ_constructed == γ_final:
                        end_time = time.time()
                        ...
                        attack_data = {
                            ...
                            "z": reqd_z_t[0],
                            "t": reqd_z_t[1],
                            "gamma_constructed": str(γ_constructed),
                            "attack_time": attack_time
                        }

                        # Append attack data to file
                        M_attack_list.append(attack_data)

                        with open(file_path, "w") as f:
                            f.write(str(M_attack_list))

                        print(f"Attack successful in RepresentInteger with M = {M_attack}, (z,t) = {reqd_z_t}")
                        print(f"Time taken for attack: {attack_time:.6f} seconds")
                        #print("γ_final=", γ_final)
                        print("γ_constructed=", γ_constructed)
                        #print(f"Attack successful in RepresentInteger with z, t = {reqd_z_t}")
            #γ_constructed = x + ω * y + j * (reqd_z_t[0] + ω * reqd_z_t[1])
...
def RepresentIntegerHeuristicESIH(M, parity=False):
    """
    Algorithm 1 (Page 8)
    ...
    def RepresentIntegerESIH(M, z, t, parity=False):
        M_prime = M - p * quadratic_norm(z, t)
    ...
    for _ in range(m**2):
        z = randint(-m, m)
        t = randint(-m, m)
        γ = RepresentIntegerESIH(M, z, t, parity=parity)
...
def strong_approximation_lattice_heuristic(N, C, D, λ, L, small_power_of_two=False):
    file_path = "StrongApproximation_attack_values.txt"
    #global DEBUG_SIGNING_KLPT
    """
    ...
    for close_v in close_vectors:
        ...
        # if DEBUG_SIGNING_KLPT:
        #     print(f"DEBUG [StrongApproximation_latticeHeuristic]: Called with cornacchia value = {M}")     
        ...
        if DEBUG_SIGNING_KLPT:
            ...
            #print("M_sa_attack=", M_sa_attack)
            prob_z_t_sa = two_squares(M_sa_attack)
            try:
                with open(file_path, "r") as f:
                    data = f.read()
                    M_sa_attack_list = ast.literal_eval(data) if data else []
            except (FileNotFoundError, SyntaxError):
                M_sa_attack_list = []
            #print("prob_z_t_sa=", prob_z_t_sa)
            reqd_z_t_sa = None
            if((prob_z_t_sa[0] == z_sa and prob_z_t_sa[1] == t_sa) or (prob_z_t_sa[0] == t_sa and prob_z_t_sa[1] == z_sa) or (prob_z_t_sa[0] == -z_sa and prob_z_t_sa[1] == -t_sa) or (prob_z_t_sa[0] == -t_sa and prob_z_t_sa[1] == -z_sa) or (prob_z_t_sa[0] == z_sa and prob_z_t_sa[1] == -t_sa) or (prob_z_t_sa[0] == -z_sa and prob_z_t_sa[1] == t_sa) or (prob_z_t_sa[0] == t_sa and prob_z_t_sa[1] == -z_sa) or (prob_z_t_sa[0] == -t_sa and prob_z_t_sa[1] == z_sa)):
                #print("attack successful in strong approximation")
                reqd_z_t_sa = (z_sa, t_sa)

        two_squares1 = Cornacchia(ZZ(M), -ZZ(ω**2))
        if two_squares1:
            xp, yp = two_squares1
            # Construct the final μ
            μ_final = N * xp + N * yp * ω + (z_sa) * j + (t_sa) * j * ω

            # If we identified a valid attack pair, construct μ_constructed
            if DEBUG_SIGNING_KLPT:
                if reqd_z_t_sa:
                    μ_constructed = N * xp + N * yp * ω + (reqd_z_t_sa[0]) * j + (reqd_z_t_sa[1]) * j * ω

                    # Compare constructed μ with final μ
                    if μ_constructed == μ_final:
                        end_time_sa = time.time()
                        attack_time_sa = end_time_sa - start_time_sa
                        attack_data = {
                            "M_sa_attack": M_sa_attack,
                            "z": reqd_z_t_sa[0],
                            "t": reqd_z_t_sa[1],
                            "mu_constructed": str(μ_constructed),
                            "attack_time": attack_time_sa  # Store attack time
                        }

                        # Append attack data to file
                        M_sa_attack_list.append(attack_data)

                        with open(file_path, "w") as f:
                            f.write(str(M_sa_attack_list))
                        #print("μ_final=", μ_final)
                        print("μ_constructed=", μ_constructed)
                        print(f"Attack successful in Strong Approximation with M = {M_sa_attack}, (z,t) = {reqd_z_t_sa}")
                        print(f"Time taken for attack: {attack_time_sa:.6f} seconds")
            break
...
def strong_approximation_lattice_heuristicESIH(N, C, D, λ, L, small_power_of_two=False):
    """
    Constructs a lattice basis and then looks for
    close vectors to the target.
...
def StrongApproximationHeuristicESIH(
    N, C, D, facT, composite_factors=None, small_power_of_two=False
):
    """
    Algorithm 2 (Page 9)
    ...
    if composite_factors:
        ...
        # If the lattice computation fails this returns None
        return strong_approximation_lattice_heuristicESIH(
            N, C, D, λ, L2, small_power_of_two=small_power_of_two
        )
    ...
    for _ in range(100):
        ...
        μ = strong_approximation_lattice_heuristicESIH(
            N, C, D, λ, L2, small_power_of_two=small_power_of_two
        )
        # Sometimes we have no valid solutions from the lattice,
        # so we try again with a new L2.
...
def strong_approximation_wrapper_KLPT(L, N, L0, facT, small_power_of_two=False):
    ...
    for _ in range(10):
        ...
        global DEBUG_SIGNING_KLPT
        DEBUG_SIGNING_KLPT = True 
        γ = RepresentIntegerHeuristic(N * L1)
        print("gamma in represent integer =", γ)
        if γ is None:
            continue
        ...
        print("mu in strong approximation=", ν)
        if ν is not None:
...
def SigningKLPT(I, Iτ):
    ...
    seen_γ = set()
    #global DEBUG_SIGNING_KLPT
    #DEBUG_SIGNING_KLPT = True  # Enable debug mode
    ...
    for _ in range(2000):
        ...
        #global DEBUG_SIGNING_KLPT
        #DEBUG_SIGNING_KLPT = True  # Enable debug mode
        γ = RepresentIntegerHeuristicESIH(N * L1)
        if γ is None:
        ...
        μ = StrongApproximationHeuristicESIH(
        ...
        #print("value of gamma in signingKLPT=", γ, flush=True)
        #print("value of mu in signingKLPT=", μ, flush=True)
        β = γ * μ
        J_prime = chi(β, L)
